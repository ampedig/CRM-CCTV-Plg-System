...
@section('content')
    <div class="flex-1 p-4 md:p-8">
        <div class="w-full">
            <!-- 1 Panel Form -->
            <form action="{{ route('transactions.update', $transaction->id) }}" method="POST" id="form-transaksi">
                @csrf
                @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div>
                                ...
                                <select class="select2-status w-full" name="payment_status" required>
                                    ...
                                    <option value="paid" {{ old('payment_status', $transaction->payment_status) === 'paid' ? 'selected' : '' }}>Paid (Lunas)</option>
                                    <option value="canceled" {{ old('payment_status', $transaction->payment_status) === 'canceled' ? 'selected' : '' }}>Canceled (Gagal)</option>
                                </select>
...
@section('scripts')
    ...
    <script>
        // Array to store selected products, pre-populated with existing details
        let selectedProducts = [
            @foreach ($transaction->details as $detail)
            {
                id: {{ $detail->product_id }},
                name: '{{ addslashes($detail->product->name) }}',
                brand: '{{ addslashes($detail->product->merk ?? "-") }}',
                category: '{{ addslashes($detail->product->category->name ?? "Tanpa Kategori") }}',
                price: {{ $detail->product->price }},
                image: '{{ $detail->product->image ? asset("storage/" . $detail->product->image) : "https://placehold.co/100x100/e2e8f0/64748b?text=" . urlencode(substr($detail->product->name, 0, 3)) }}',
                qty: {{ $detail->quantity }}
            },
            @endforeach
        ];

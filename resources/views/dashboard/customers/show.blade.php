...
@section('content')
    <div class="flex-1 p-8">
        <div class="max-w-screen-2xl mx-auto">
            ...
            <!-- Back Button -->
            <a href="{{ route('customers.index') }}" class="inline-flex items-center gap-2 text-sm font-semibold text-slate-500 hover:text-slate-800 transition mb-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-1 flex flex-col gap-6">
                    <div class="bg-white border border-slate-200 rounded-2xl p-6 flex flex-col items-center text-center">
                        ...
                        <!-- Initials Avatar -->
                        <div class="w-20 h-20 bg-brand-50 rounded-3xl flex items-center justify-center text-brand-600 font-bold text-3xl mb-4 border border-brand-100">
                        ...
                        <a href="https://wa.me/{{ $customer->whatsapp_number }}" target="_blank"
                            class="btn btn-primary w-full rounded-xl flex items-center justify-center gap-2 py-2.5 font-semibold text-sm mb-6">
                        ...
                        <div class="w-full border-t border-slate-100 pt-4 text-left flex flex-col gap-3.5 text-sm">
                            ...
                            <div class="flex justify-between items-center pb-3.5 border-b border-slate-50">
                                ...
                                <span class="text-slate-700 font-semibold">{{ $customer->date_of_birth ? \Carbon\Carbon::parse($customer->date_of_birth)->translatedFormat('d F Y') : '-' }}</span>
                            </div>
                            <div class="flex justify-between items-center pb-3.5 border-b border-slate-50">
                                <span class="text-slate-400 font-medium">Usia</span>
                                <span class="text-slate-700 font-semibold">{{ $customer->date_of_birth ? \Carbon\Carbon::parse($customer->date_of_birth)->age . ' Tahun' : '-' }}</span>
                            </div>
                            <div class="flex justify-between items-center pb-3.5 border-b border-slate-50">
                                <span class="text-slate-400 font-medium">Status Akun</span>
                                @if ($customer->is_active)
                                    <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-600 border border-emerald-100">Aktif</span>
                                @else
                                    <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-rose-50 text-rose-600 border border-rose-100">Nonaktif</span>
                                @endif
                            </div>
                            <div class="flex justify-between items-center pb-3.5 border-b border-slate-50">
                                <span class="text-slate-400 font-medium">Status Pembayaran</span>
                                @if ($customer->payment_status === 'Lunas')
                                    <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-green-50 text-green-600 border border-green-100">Lunas</span>
                                @else
                                    <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-amber-50 text-amber-600 border border-amber-100">Belum Lunas</span>
                                @endif
                            </div>
                            <div class="flex justify-between items-center pb-3.5 border-b border-slate-50">
                                <span class="text-slate-400 font-medium">Skor Calon Pembeli</span>
                                @if (strtolower($customer->lead_score_status) === 'hot')
                                    <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-rose-50 text-rose-600 border border-rose-100">Hot</span>
                                @elseif (strtolower($customer->lead_score_status) === 'warm')
                                    <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-amber-50 text-amber-600 border border-amber-100">Warm</span>
                                @else
                                    <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-slate-100 text-slate-600 border border-slate-200">Cold</span>
                                @endif
                            </div>
                            <div class="flex justify-between items-center pb-3.5 border-b border-slate-50">
                                <span class="text-slate-400 font-medium">Frekuensi Konsultasi</span>
                                <span class="text-slate-700 font-semibold">{{ $customer->consultation_frequency }} kali</span>
                            </div>
                            <div class="flex justify-between items-center pb-3.5 border-b border-slate-50">
                            ...
                            <div class="flex justify-between items-center">
                                ...
                                <span class="text-slate-700 font-semibold capitalize">{{ $customer->last_product_interest ?? 'Belum ada data' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="bg-white border border-slate-200 rounded-2xl overflow-hidden">
                        ...
                        <div class="p-5 border-b border-slate-100 flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-xs text-slate-400 font-medium mt-0.5">Menampilkan total {{ $customer->transaction_count }} transaksi pembelian</p>
                            </div>
                        </div>
                            ...
                            <table class="w-full text-left border-collapse">
                                <thead class="bg-slate-50 text-slate-500 uppercase text-xs font-semibold tracking-wider">
                                    <tr>
                                        ...
                                        <th class="px-6 py-4 border-b border-slate-100 font-semibold text-slate-800 uppercase tracking-wider td-nowrap">Tanggal</th>
                                        <th class="px-6 py-4 border-b border-slate-100 font-semibold text-slate-800 uppercase tracking-wider td-nowrap">Total Belanja</th>
                                        <th class="px-6 py-4 border-b border-slate-100 font-semibold text-slate-800 uppercase tracking-wider td-nowrap">Status Pembayaran</th>
